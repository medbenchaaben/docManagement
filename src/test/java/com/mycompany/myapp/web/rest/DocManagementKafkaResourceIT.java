...
class DocManagementKafkaResourceIT {
    ...
    void setup() {
        ...
        DocManagementKafkaResource kafkaResource = new DocManagementKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/doc-management-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/doc-management-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
